# Installation - skill-creator-anthropic
...
```bash
cd ~/.claude/skills/skill-creator-anthropic
ls -la
```
...
```bash
cd ~/.claude/skills/skill-creator-anthropic/src
npm install
# or
bun install
```
...
1. Copy the example configuration:
   ```bash
   cp ~/.claude/skills/skill-creator-anthropic/src/config/example.env ~/.claude/.env
   ```
...
claude --help | grep skill-creator-anthropic
