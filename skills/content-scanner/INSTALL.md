...
```bash
cd ~/.claude/skills/content-scanner
ls -la
```
...
```bash
cd ~/.claude/skills/content-scanner/src
npm install
# or
bun install
```
...
1. Copy the example configuration:
   ```bash
   cp ~/.claude/skills/content-scanner/src/config/example.env ~/.claude/.env
   ```
...
claude --help | grep content-scanner
