# Installation - session-persistence
...
```bash
cd ~/.claude/skills/session-persistence
ls -la
```
...
```bash
cd ~/.claude/skills/session-persistence/src
npm install
# or
bun install
```
...
1. Copy the example configuration:
   ```bash
   cp ~/.claude/skills/session-persistence/src/config/example.env ~/.claude/.env
   ```
...
claude --help | grep session-persistence
