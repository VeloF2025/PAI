# Installation - proactive-scanner
...
```bash
cd ~/.claude/skills/proactive-scanner
ls -la
```
...
```bash
cd ~/.claude/skills/proactive-scanner/src
npm install
# or
bun install
```
...
1. Copy the example configuration:
   ```bash
   cp ~/.claude/skills/proactive-scanner/src/config/example.env ~/.claude/.env
   ```
...
claude --help | grep proactive-scanner
