# Installation - async-orchestration
...
```bash
cd ~/.claude/skills/async-orchestration
ls -la
```
...
```bash
cd ~/.claude/skills/async-orchestration/src
npm install
# or
bun install
```
...
1. Copy the example configuration:
   ```bash
   cp ~/.claude/skills/async-orchestration/src/config/example.env ~/.claude/.env
   ```
...
claude --help | grep async-orchestration
