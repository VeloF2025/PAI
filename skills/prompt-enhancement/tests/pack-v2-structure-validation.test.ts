/**
 * Pack v2.0 Structure Validation Tests
 *
 ...
 *
 * CRITICAL: These tests validate the ACTUAL Pack v2.0 requirements:
 ...
 *
 * TDD APPROACH: These tests should FAIL first, then we implement until they PASS.
 */
...
describe('Pack v2.0 Directory Structure', () => {
  test('🔴 MUST have README.md (not PACK_README.md)', () => {
    ...
    if (existsSync(wrongPath)) {
      console.log(`❌ Found old format: PACK_README.md (should be README.md)`);
    }
  ...
  test('🔴 MUST have INSTALL.md (not PACK_INSTALL.md)', () => {
    ...
    if (existsSync(wrongPath)) {
      console.log(`❌ Found old format: PACK_INSTALL.md (should be INSTALL.md)`);
    }
  ...
  test('🔴 MUST have VERIFY.md (not PACK_VERIFY.md)', () => {
    ...
    if (existsSync(wrongPath)) {
      console.log(`❌ Found old format: PACK_VERIFY.md (should be VERIFY.md)`);
    }
...
describe('Real Code Files in src/', () => {
  ...
  test('🔴 Code files MUST NOT be simplified (complete implementations)', () => {
    ...
    const simplified = codeFiles.filter(f => {
      // When recursive: true, use parentPath (full path) and combine with file name
      // parentPath contains the full directory path where the file is located
