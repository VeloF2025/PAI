...
import { AnthropicBlogScraper, GitHubReleasesScraper, DanielRepoScraper, AnthropicDocsScraper, YouTubeRSSScraper, RSSFeedScraper, AnthropicCookbookScraper } from "./scrapers/index.ts";
...
async function scanSource(source: Source): Promise<Finding[]> {
  ...
  switch (source.id) {
    ...
    case "anthropic-docs":
      return await scrapeAnthropicDocs(source);

    case "daniel-repo":
      return await scrapeDanielRepo(source);

    case "anthropic-cookbook":
      return await scrapeAnthropicCookbook(source);

    case "indiedevdan-youtube":
    case "aijason-youtube":
      return await scrapeYouTubeChannel(source);

    case "simon-willison-blog":
    case "langchain-blog":
    case "trailofbits-blog":
    case "portswigger-research":
...
/**
 * Scrape Anthropic blog using direct HTML parsing
 *
 * Since WebFetch MCP isn't available via CLI and Anthropic doesn't have RSS,
 * we fetch and parse the HTML directly.
 */
async function scrapeAnthropicBlog(source: Source): Promise<Finding[]> {
  try {
    verbose(`  🌐 Fetching Anthropic news page...`);

    const response = await fetch(source.url);
    if (!response.ok) {
      verbose(`  ❌ Failed to fetch page: HTTP ${response.status}`);
      return [];
    }
    ...
    // Anthropic's news page embeds article data as JSON in the HTML
    const articles = [];

    // Look for JSON data in <script> tags or __NEXT_DATA__
    const nextDataMatch = html.match(/<script[^>]*id="__NEXT_DATA__"[^>]*>(.*?)<\/script>/s);
    if (nextDataMatch) {
      try {
        const nextData = JSON.parse(nextDataMatch[1]);

        // Navigate through Next.js data structure to find posts
        const pageProps = nextData?.props?.pageProps;
        if (pageProps) {
          // Look for posts array in various possible locations
          const posts = pageProps.posts || pageProps.articles || pageProps.data?.posts || [];

          for (const post of posts.slice(0, 10)) {
            if (!post || post._type !== 'post') continue;
            ...
            articles.push({
              ...
              date: post.publishedOn || post.date || new Date().toISOString(),
              summary: post.summary || post.description || '',
              keywords: post.subjects?.map((s: any) => s.label.toLowerCase()) || []
            });
          }
        }
      } catch (jsonError) {
        verbose(`  ⚠️  Failed to parse Next.js data: ${jsonError}`);
      }
    }

    // Fallback: try to extract from article links if JSON parsing failed
    ...
    if (articles.length === 0) {
      verbose(`  ⚠️  No articles found on page`);
      return [];
    }
...
async function scrapeDanielRepo(source: Source): Promise<Finding[]> {
  try {
    verbose(`  📚 Fetching commits using gh CLI...`);
    ...
    verbose(`  ✅ Found ${findings.length} commits from ${repo}`);
    const highCount = findings.filter(f => f.relevance === "high").length;
    const medCount = findings.filter(f => f.relevance === "medium").length;
    const lowCount = findings.filter(f => f.relevance === "low").length;
    verbose(`     High: ${highCount}, Medium: ${medCount}, Low: ${lowCount}`);
    ...
    verbose(`  ❌ Error scraping Daniel's repo: ${error}`);
    if (error instanceof Error && error.message.includes("gh: not found")) {
      verbose(`  💡 Hint: Install GitHub CLI (gh) - see https://cli.github.com/`);
    }
    return [];
...
/**
 * Scrape Anthropic Cookbook repository for examples and techniques
 */
async function scrapeAnthropicCookbook(source: Source): Promise<Finding[]> {
  try {
    verbose(`  📚 Fetching cookbook commits using gh CLI...`);
    ...
    const repo = repoMatch[1]; // "anthropics/anthropic-cookbook"

    // Use AnthropicCookbookScraper to fetch and process commits
    const findings = await AnthropicCookbookScraper.scrape(repo, source.id, {
      maxCommits: 20,
      includeFiles: true
    });
    ...
    verbose(`  ❌ Error scraping Anthropic Cookbook: ${error}`);
    if (error instanceof Error && error.message.includes("gh: not found")) {
      verbose(`  💡 Hint: Install GitHub CLI (gh) - see https://cli.github.com/`);
    }
    return [];
 ...
 */
async function scrapeAnthropicDocs(source: Source): Promise<Finding[]> {
  try {
    verbose(`  📚 Fetching Anthropic documentation release notes...`);

    // Use AnthropicDocsScraper to fetch and process documentation updates
    const findings = await AnthropicDocsScraper.scrape(source.id, {
      maxUpdates: 50
    });

    verbose(`  ✅ Found ${findings.length} documentation updates`);
    const highCount = findings.filter(f => f.relevance === "high").length;
    const medCount = findings.filter(f => f.relevance === "medium").length;
    const lowCount = findings.filter(f => f.relevance === "low").length;
    verbose(`     High: ${highCount}, Medium: ${medCount}, Low: ${lowCount}`);
    ...
    verbose(`  ❌ Error scraping Anthropic docs: ${error}`);
    return [];
  }
}

/**
 * Scrape YouTube channel for AI-related videos
 */
async function scrapeYouTubeChannel(source: Source): Promise<Finding[]> {
  try {
    if (!source.metadata?.channelId) {
      verbose(`  ❌ No channel ID configured for ${source.name}`);
      return [];
    }

    verbose(`  📺 Fetching latest videos from YouTube channel...`);

    // Use YouTubeRSSScraper to fetch and process videos
    const findings = await YouTubeRSSScraper.scrape(
      source.id,
      source.metadata.channelId,
      source.name,
      {
        maxVideos: 10
      }
    ...
    verbose(`  ✅ Found ${findings.length} videos`);
    const highCount = findings.filter(f => f.relevance === "high").length;
    const medCount = findings.filter(f => f.relevance === "medium").length;
    const lowCount = findings.filter(f => f.relevance === "low").length;
    verbose(`     High: ${highCount}, Medium: ${medCount}, Low: ${lowCount}`);
    ...
    verbose(`  ❌ Error scraping YouTube channel: ${error}`);
    return [];
  }
}

/**
 * Scrape RSS/Atom feed for blog posts
 */
async function scrapeRSSFeed(source: Source): Promise<Finding[]> {
  try {
    verbose(`  📰 Fetching RSS feed...`);

    // Determine feed URL based on source ID
    const feedUrl = getRSSFeedUrl(source);

    if (!feedUrl) {
      verbose(`  ❌ No RSS feed URL configured for ${source.name}`);
      return [];
    }

    // Use RSSFeedScraper to fetch and process posts
    const findings = await RSSFeedScraper.scrape(
      source.id,
      feedUrl,
      source.name,
      {
        maxPosts: source.filters?.max_posts || 10,
        keywords: source.filters?.keywords,
      }
    );

    verbose(`  ✅ Found ${findings.length} posts`);
    const highCount = findings.filter(f => f.relevance === "high").length;
    const medCount = findings.filter(f => f.relevance === "medium").length;
    const lowCount = findings.filter(f => f.relevance === "low").length;
    verbose(`     High: ${highCount}, Medium: ${medCount}, Low: ${lowCount}`);
    ...
    verbose(`  ❌ Error scraping RSS feed: ${error}`);
    return [];
  }
}
