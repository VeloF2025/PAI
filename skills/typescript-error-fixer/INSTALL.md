# Installation - typescript-error-fixer
...
```bash
cd ~/.claude/skills/typescript-error-fixer
ls -la
```
...
```bash
cd ~/.claude/skills/typescript-error-fixer/src
npm install
# or
bun install
```
...
1. Copy the example configuration:
   ```bash
   cp ~/.claude/skills/typescript-error-fixer/src/config/example.env ~/.claude/.env
   ```
...
claude --help | grep typescript-error-fixer
