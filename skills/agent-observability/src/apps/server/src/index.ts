...
const server = Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    // Routing Analytics API endpoints

    // GET /api/routing/summary - Current routing statistics
    if (url.pathname === '/api/routing/summary' && req.method === 'GET') {
      const result = getRoutingSummary();
      return new Response(JSON.stringify(result), {
        headers: { ...headers, 'Content-Type': 'application/json' }
      });
    }

    // GET /api/routing/savings - Savings reports
    if (url.pathname === '/api/routing/savings' && req.method === 'GET') {
      const period = url.searchParams.get('period') as 'today' | 'week' | 'month' || 'today';
      const result = getSavingsReports(period);
      return new Response(JSON.stringify(result), {
        headers: { ...headers, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (url.pathname === '/api/routing/decisions' && req.method === 'GET') {
      ...
      const result = getRecentDecisions(limit);
      return new Response(JSON.stringify(result), {
        headers: { ...headers, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (url.pathname === '/api/routing/retries' && req.method === 'GET') {
      const limit = parseInt(url.searchParams.get('limit') || '20');
      const result = getRetryDetections(limit);
      return new Response(JSON.stringify(result), {
        headers: { ...headers, 'Content-Type': 'application/json' }
      });
    }
    ...
    if (url.pathname === '/api/routing/trends' && req.method === 'GET') {
      const hours = parseInt(url.searchParams.get('hours') || '24');
      const result = getCostTrends(hours);
      return new Response(JSON.stringify(result), {
        headers: { ...headers, 'Content-Type': 'application/json' }
      });
    }

    // GET /api/routing/distribution - Model distribution statistics
    if (url.pathname === '/api/routing/distribution' && req.method === 'GET') {
    ...
    // GET /api/routing/accuracy - Routing accuracy metrics
    if (url.pathname === '/api/routing/accuracy' && req.method === 'GET') {
      const result = getAccuracyMetrics();
      return new Response(JSON.stringify(result), {
        headers: { ...headers, 'Content-Type': 'application/json' }
      });
    }
...
console.log(`📈 Routing analytics: http://localhost:${server.port}/api/routing/*`);