...
```bash
cd ~/.claude/skills/example-skill
ls -la
```
...
```bash
cd ~/.claude/skills/example-skill/src
npm install
# or
bun install
```
...
1. Copy the example configuration:
   ```bash
   cp ~/.claude/skills/example-skill/src/config/example.env ~/.claude/.env
   ```
...
claude --help | grep example-skill
