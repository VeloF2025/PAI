# Installation - mcp-troubleshooter
...
```bash
cd ~/.claude/skills/mcp-troubleshooter
ls -la
```
...
```bash
cd ~/.claude/skills/mcp-troubleshooter/src
npm install
# or
bun install
```
...
1. Copy the example configuration:
   ```bash
   cp ~/.claude/skills/mcp-troubleshooter/src/config/example.env ~/.claude/.env
   ```
...
claude --help | grep mcp-troubleshooter
